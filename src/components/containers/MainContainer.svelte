<script lang="ts">
	...
	import EventEntry from "../entries/EventEntry.svelte";
	...
	})("When the impostor is suspicious");
</script>

<div class="MainContainer overflow-wrapper">
	<div class="flex-overflow">
		{#if visible}
			<EventEntry event={testEvent} />
			<h1 in:typewriter={{ speed: 100 }}>subcrypt</h1>
			<p>
				<span in:typewriter={{ delay: 1000 }}
					>A text-based adventure by&nbsp;</span
				><a
					draggable="false"
		...
		{/if}
		{#if showAll}
			<button on:click={newGame} style="margin: 0.5rem auto 1rem"
				>New Game</button
			>
			<SettingsDisplay />
		{/if}
